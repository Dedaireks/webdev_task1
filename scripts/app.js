...
function rerender() {
  ...
  for (const index in todos) {
    ...
    element.innerHTML = `<div class="todo__day">Дело ${Number(index) + 1}</div>
              ...
              <input type="text" class="todo__input" onblur="editTodo(${index}, this)" 
              onkeyup="if(event.keyCode === 13) editTodo(${index}, this)" value="${todos[index]}"/>
...
function editTodo (index, input) {
  todos[index] = input.value || todos[index];
  rerender()
  saveData();
}
